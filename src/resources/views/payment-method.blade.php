    ...
    <div class="payment-method__container">
        ...
        <form class="payment-method__form" action="{{ route('payment.process') }}" method="POST">
            ...
            @foreach ($paymentMethods as $method)
                <div>
                    <input class="payment-method__input" type="radio" id="payment_{{ $method->id }}" name="payment_method" value="{{ $method->id }}" required>
                    <label class="payment-method__label "for="payment_{{ $method->id }}">{{ $method->payment_method }}</label>
                </div>
            @endforeach
